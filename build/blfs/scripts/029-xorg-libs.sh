#!/bin/bash
# Generated from BLFS 13.0-systemd (x/x7lib.html) — runs inside chroot as root
...
cd /sources/blfs
rm -rf xorg-libs-work
mkdir xorg-libs-work
cd xorg-libs-work
...
mkdir build
cd build
for t in $(grep -v '^#' ../lib-7.md5 | awk '{print $2}'); do
  cp -v /sources/blfs/$t .
done
md5sum -c ../lib-7.md5
